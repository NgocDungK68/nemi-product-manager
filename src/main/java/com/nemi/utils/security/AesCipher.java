...
public class AesCipher {
    ...
    public CipherText encrypt(PlainText plainText, Password password) throws Exception {
        ...
        cipher.init(Cipher.ENCRYPT_MODE, key, getAlgorithm().getAlgorithmParameterSpec(iv)); //iv là vector khởi tạo để tăng tính bảo mật (mỗi lần mã hóa sẽ khác nhau)
    ...
    public PlainText decrypt(CipherText cipherText, Password password) throws Exception {
        try {
            ...
            throw new IllegalArgumentException("The provided cipherText is too short.", e);
        }
    }
}